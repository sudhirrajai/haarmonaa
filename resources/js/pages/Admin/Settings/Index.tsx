...
export default function Index({ settings }: SettingsProps) {
  ...
  const [activeTab, setActiveTab] = useState<'branding' | 'navigation' | 'payments_shipping' | 'seo' | 'social'>('branding');
  ...
  const tabs = [
    { id: 'branding', label: 'Branding & Store Details', icon: Store, hint: 'Logos, Favicon, Brand & Contact Details' },
    { id: 'navigation', label: 'Navigation & Header', icon: Menu, hint: 'Top Announcement Bar & Header Menu Links' },
    { id: 'payments_shipping', label: 'Shipping & Payments', icon: CreditCard, hint: 'Currency, Taxes, Shipping Rules & Razorpay' },
    { id: 'seo', label: 'SEO & Verification', icon: Globe, hint: 'Meta Tags, Social Sharing & Search Indexing' },
    { id: 'social', label: 'Social Channels', icon: Share2, hint: 'Social Profiles & Community Links' },
  ] as const;
  ...
  return (
    <AdminLayout title="Store Settings">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-[13px] text-gray-500 mt-1">
            Manage your brand assets, header menus, payment gateways, shipping rules, and search engine optimization.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-300 rounded-xl text-xs font-bold transition-all shadow-2xs shrink-0"
          >
            <Layout className="w-4 h-4 text-amber-700" />
            <span className="hidden sm:inline">Edit Homepage Sections</span>
          </Link>
          ...
          <button
            ...
            disabled={saving}
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#111111] hover:bg-[#d0473e] text-white rounded-xl text-xs font-bold transition-all shadow-xs disabled:opacity-50 cursor-pointer shrink-0"
          >
      ...
      <div className="bg-white rounded-2xl border border-gray-200/80 p-2 shadow-2xs">
      ...
      <form onSubmit={handleSubmit} className="w-full space-y-8">
            ...
            <div className="bg-white p-6 sm:p-8 rounded-[10px] border border-gray-200/80 shadow-2xs space-y-6">
              ...
              <div className="p-3 bg-gray-50 border border-gray-200/70 rounded-xl space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => handleQuickAddCategory('Best Sellers', '/shop?filter=best-sellers')}
                    className="px-2.5 py-1 bg-white hover:bg-gray-100 border border-gray-200 rounded-lg text-[11px] font-semibold text-gray-800 transition-all cursor-pointer shadow-2xs"
                  >
                    + Best Sellers
                  </button>
                  <button
                    type="button"
                    onClick={() => handleQuickAddCategory('New Arrivals', '/shop?filter=new-arrivals')}
                    className="px-2.5 py-1 bg-white hover:bg-gray-100 border border-gray-200 rounded-lg text-[11px] font-semibold text-gray-800 transition-all cursor-pointer shadow-2xs"
                  >
                    + New Arrivals
                  </button>
              ...
              <div className="space-y-3">
                {formData.header_nav_items.map((item: any, index: number) => (
                  <div
                    key={item.id}
                    className="p-3.5 bg-gray-50/80 border border-gray-200 rounded-xl space-y-3 shadow-2xs hover:border-gray-300 transition-all"
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div className="flex items-center gap-2 flex-1">
                        ...
                        <div className="flex flex-col gap-0.5">
                          <button
                            ...
                            className="p-1 hover:bg-gray-200 disabled:opacity-30 rounded text-gray-600 transition-colors cursor-pointer"
                            title="Move Up"
                          >
                        ...
                        <div className="flex-1 sm:max-w-xs">
                          <input
                            ...
                            placeholder="Link Label (e.g. Earrings)"
                            className="w-full bg-white border border-gray-200 rounded-[8px] py-1.5 px-3 text-xs font-bold text-gray-900 focus:outline-hidden focus:border-black"
                          />
                        </div>
                        ...
                        <div className="flex-1">
                          <input
                            ...
                            placeholder="/category/earrings or https://..."
                            className="w-full bg-white border border-gray-200 rounded-[8px] py-1.5 px-3 text-xs text-gray-700 font-mono focus:outline-hidden focus:border-black"
                          />
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 self-end sm:self-center">
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 text-[11px] font-bold text-amber-800 hover:text-black bg-amber-50 hover:bg-amber-100 border border-amber-200 px-2 py-1 rounded-lg transition-colors cursor-pointer"
                          title="Add sub-menu link"
                        >
                        ...
                        <label className="flex items-center gap-1.5 text-xs text-gray-600 cursor-pointer select-none">
                          <input
                            ...
                            onChange={(e) => handleUpdateNavItem(item.id, 'is_enabled', e.target.checked)}
                            className="rounded text-black focus:ring-0 w-3.5 h-3.5"
                          />
                    ...
                    {item.children && item.children.length > 0 && (
                      <div className="mt-2 pt-2.5 border-t border-gray-200/70 pl-6 sm:pl-8 space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-bold text-amber-800 uppercase tracking-wider flex items-center gap-1">
                            <CornerDownRight className="w-3 h-3" />
                            <span>Dropdown Sub-Menu Links ({item.children.length})</span>
                          </span>

                          {/* Quick Add Subcategory Presets */}
                          {item.url && item.url.includes('rings') && (
                            <div className="flex items-center gap-1.5">
                              <button
                                type="button"
                                onClick={() => handleAddCategorySub(item.id, 'Anti Tarnish Rings', '/category/rings/anti-tarnish-rings')}
                                className="text-[10px] font-semibold bg-amber-50 hover:bg-amber-100 text-amber-900 px-2 py-0.5 rounded border border-amber-200 transition-colors"
                              >
                                + Anti Tarnish Rings
                              </button>
                            </div>
                        ...
                        <div className="space-y-1.5">
                          {item.children.map((sub: any, subIndex: number) => (
                            <div
                              key={sub.id}
                              className="p-2 bg-white border border-gray-200/90 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between gap-2 shadow-2xs hover:border-amber-400 transition-all"
                            >
                              <div className="flex items-center gap-2 flex-1">
                                <div className="flex flex-col gap-0.5">
                                  <button
                                    ...
                                    className="p-0.5 hover:bg-gray-100 disabled:opacity-30 rounded text-gray-500 transition-colors cursor-pointer"
                                  >
                                ...
                                <div className="w-36 sm:w-44">
                                  <input
                                    ...
                                    className="w-full bg-gray-50 border border-gray-200 rounded-[6px] py-1 px-2 text-xs font-semibold text-gray-900 focus:outline-hidden focus:border-black"
                                  />
                                </div>
                                ...
                                <div className="flex-1">
                                  <input
                                    ...
                                    className="w-full bg-gray-50 border border-gray-200 rounded-[6px] py-1 px-2 text-xs text-gray-700 font-mono focus:outline-hidden focus:border-black"
                                  />
                                </div>
                              </div>

                              <div className="flex items-center gap-2 self-end sm:self-center">
                                <label className="flex items-center gap-1 text-[10.5px] text-gray-600 cursor-pointer select-none">
                                  <input
                                    type="checkbox"
                                    checked={sub.is_enabled !== false}
                                    onChange={(e) => handleUpdateSubItem(item.id, sub.id, 'is_enabled', e.target.checked)}
                                    className="rounded text-black focus:ring-0 w-3 h-3"
                                  />
                                  <span>Visible</span>
                                </label>
            ...
            <div className="bg-white p-6 sm:p-8 rounded-[10px] border border-gray-200/80 shadow-2xs space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2 pt-2">
                  <SingleImageUploader
                    label="Default Social Share Image (OpenGraph Image)"
                    hint="Recommended: 1200×630 px (PNG/JPG) for WhatsApp, Facebook, iMessage previews"
                    placeholder="Upload social share preview image or paste URL..."
                    value={formData.og_default_image}
                    onChange={(url) => setFormData({ ...formData, og_default_image: url })}
                  />
                </div>
              </div>
            </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-200/80">
          ...
          <button
            type="submit"
            disabled={saving}
            className="inline-flex items-center gap-2 px-8 py-3.5 bg-[#111111] hover:bg-[#d0473e] text-white rounded-xl text-xs font-bold uppercase tracking-wider transition-all shadow-xs disabled:opacity-50 cursor-pointer"
          >
            <Save className="w-4 h-4" />
            <span>{saving ? 'Saving...' : 'Save Settings'}</span>
          </button>
